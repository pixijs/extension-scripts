...
const globals = {
    ...
    ...builtInPackages,
    ...(pkg.extensionConfig?.globals ?? {}),
};

const source = pkg.source ?? 'src/index.ts';
const namespace = pkg.extensionConfig?.namespace ?? 'PIXI.extension';
const bundle = path.join(process.cwd(), pkg.extensionConfig?.bundle
    ?? `dist/${pkg.name.replace(/\//g, '-').replace(/[^\w\d-_]/g, '')}.js`);
